...
class CceModelNews extends CceModelCce {
       ...
       function saveTopNews($pnewstext)
       {
 	    ...
 	    function updatethoughts_day($id,$day,$message,$reference)
       {
 					 $q = "UPDATE #__todaythoughts SET `day`=trim('".$day."'),`message`='".mysql_real_escape_string($message)."',`reference`='".mysql_real_escape_string($reference)."' WHERE `id`=".$id;
                $db = & JFactory::getDBO();
                $db->setQuery($q);
                if(!$db->query())
                        return false;
 		...
 		function gettodaythoughts(&$rec)
        {
                $q = "SELECT `id`,`day`,`message`,`reference` FROM #__todaythoughts order by `id`";
                $db = & JFactory::getDBO();
                $db->setQuery($q);
      	...
      	function editthoughts($day,&$rec)
        {
                $q = "SELECT `id`,`day`,`message`,`reference` FROM #__todaythoughts where day='".$day."'";
                $db = & JFactory::getDBO();
                $db->setQuery($q);
                $rec = $db->loadObject();
                if($rec==null)
