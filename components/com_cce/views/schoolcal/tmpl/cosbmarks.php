<?php
	...
	$arecs=$this->model->getCoScholasticBActivities();
...
<?php
	echo '<center><h1>'.$crec->coursename.' - Co-Curricular Activities </h1><h3>['.$trec->term.']</h3></center>';
?>
<hr /> <br />
<?php
	   echo '<div  align="right"><ul>';
	   foreach($arecs as $arec){
		$link = JRoute::_('index.php?option='.JRequest::getVar('option').'&controller=cosmarks&view=cosmarks&layout=cosbmarks&termid='.$termid.'&activityid='.$arec->id.'&courseid='.$courseid);
...
?>		<form action="<?php echo JRoute::_('index.php?option=com_cce&controller=cosmarks&task=savecosbmarks'); ?>" method="POST" name="adminForm">
<?php
...
<?php
		$r = $this->model->getCoSBMarks($srec->id,$arec->id,$courseid,$termid,$data);	
?>			
...
<?php
		...
		<input type="hidden" name="layout" value="cosbmarks" />
		<input type="hidden" name="task" value="savecosbmarks" />
</form>
<?php
	break;
	}
