<?php
	...
	$arecs=$this->model->getCoScholasticAActivities();
...
<?php
	echo '<center><h1>'.$crec->coursename.' - Wellness and Yoga/Holistic Exercise</h1><h3>['.$trec->term.']</h3></center>';
?>
<hr /> <br />
<?php
	   echo '<div  align="right"><ul>';
	   foreach($arecs as $arec){
		$link = JRoute::_('index.php?option='.JRequest::getVar('option').'&controller=cosmarks&view=cosmarks&layout=cosamarks&termid='.$termid.'&activityid='.$arec->id.'&courseid='.$courseid);
...
?>		<form action="<?php echo JRoute::_('index.php?option=com_cce&controller=cosmarks&task=savecosamarks'); ?>" method="POST" name="adminForm">
<?php
...
<?php
		$r = $this->model->getCoSAMarks($srec->id,$arec->id,$courseid,$termid,$data);	
?>			
...
<?php
		...
		<input type="hidden" name="layout" value="cosamarks" />
		<input type="hidden" name="task" value="savecosamarks" />
</form>
<?php
	break;
	}
