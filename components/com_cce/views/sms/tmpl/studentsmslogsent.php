...
$s = $model->getSMSSentStatusQSent($logid,$recs);
...
foreach($recs as $rec)
{
	...
	echo "<tr>";
	echo "<td>$rec->cdate</td><td>$rec->ctime</td><td>$rec->smstext</td><td>$stype</td><td>$srec->firstname</td><td>$class</td><td>$rec->mobile</td><td>$rec->errortext</td></tr>";
	$i++;
}
?>
							 </tbody>
