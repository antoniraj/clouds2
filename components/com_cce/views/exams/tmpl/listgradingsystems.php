...
<div class="row-fluid sortable">
	<div class="box span12">
		<div class="box-header well" data-original-title>
			<h2><i class="icon-edit"></i> GRADING SYSTEMS</h2>
		</div>
		<div class="box-content">
			<table class="table table-striped table-bordered bootstrap-datatable datatable">
				...
				<?php
					$i=1;
					foreach($gss as $gsrec){
   						$subjectgslink= JRoute::_('index.php?option='.JRequest::getVar('option').'&controller=exams&view=exams&task=savesubjectgradingsystem&subjectid='.$subjectid.'&gsid='.$gsrec->id.'&cbid='.$cbid.'&Itemid='.$Itemid);
						echo '<tr>';
						echo '<td>'.$i++.'</td>';
						echo '<td>'.htmlspecialchars($gsrec->title).'</td>';
						echo '<td><a class="btn btn-small btn-info" style="width:50px;" href="'.$subjectgslink.'"><i class="icon-plus-sign"></i>Select</a></td>';
						echo '</tr>';
					}
				?>
				</tbody>
