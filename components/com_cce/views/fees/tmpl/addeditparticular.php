<?php
	...
	$rs = $model->getFeeParticular($fpid,$rec);
	if($rs==false) {
		$rec->id = -1;
		$rec->name='';
		$rec->description='';
	...
	$fpItemid = $model->getMenuItemid('manageschool','Fee Particulars');
   	...
   	$fclink= JRoute::_('index.php?option='.JRequest::getVar('option').'&controller=fees&view=fees&task=display&layout=feecategory&eon='.$eon.'&fstid='.$fstid.'&fcid='.$fcid.'&Itemid='.$Itemid);
	$app =& JFactory::getApplication();
...
<center>
	<table width="100%" cellpadding="10">
                ...
                <tr style="border:none;" >
                        <td align="right" style="border:none;" >Fee Head</td>
                        <td align="left" style="border:none;" >
<?php
	if($rec->name) { ?>
                                <input type="text" readonly id="description" name="description" size="50" maxlength="100" value="<?php echo $rec->description; ?>" />
	<?php } else { ?>
  ...
  <select id="selectError" data-rel="chosen" style="width:230px;" name="accountid">
		...
		$accounts = $model->getFeeAccounts();
                ...
                <tr style="border:none;" >
                        <td align="right" colspan="2" style="border:none;" >
        ...
        <input type="hidden" name="task" id="task" value="savefeeparticular" />
