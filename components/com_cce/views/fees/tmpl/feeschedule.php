...
<b style="font: bold 15px Georgia, serif;">FEE COLLECTION SCHEDULE</b>
  ...
  <?php
        ...
        ?>
				<form class="form-horizontal" action="<?php echo JRoute::_('index.php'); ?>" method="POST" name="adminForm<?php echo $rec->id; ?>">
        <tr>
                 <td><?php echo $i; ?></td>
                 <td>
		...
		?>
			<table>
			<tr style="border:0px;"><td style="border:0px;">Class</td><td style="border:0px;">Date</td></tr>
			<?php
			foreach($crecs as $crec){  
				...
				?>
					...
					<td style="border:0px;">
					<input type="text" name="fdate[<?php echo $crec->id.'-'.$rec->id.'-'.$fsid; ?>]" class="datepicker" id="<?php echo $crec->id.$rec->id; ?>" value="<?php echo JArrayHelper::indianDate($fdate); ?>">
					</td>
					<td style="border:0px;">
					<!--	<input type="text" name="fine[<?php echo $crec->id.'-'.$rec->id.'-'.$fsid; ?>]" size="7" value="<?php echo $fine; ?>" size="5" /> -->
						<input type="hidden" name="fine[<?php echo $crec->id.'-'.$rec->id.'-'.$fsid; ?>]" size="7" value="0" size="5" />
					</td>
				</tr>
	<?php 
		if($rec->groupbased=="1") {
	...
	<input type="hidden" name="gid[<?php echo $crec->id.'-'.$rec->id; ?>]" value="<?php echo $gid; ?>" />
	<input type="hidden" name="fside[<?php echo $crec->id.'-'.$rec->id; ?>]" value="<?php echo $fsid; ?>" />
