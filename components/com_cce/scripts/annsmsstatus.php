...
<?php
        ...
        function sms_status_api($msgid,$mobiles,&$res,&$st){
                //Please Enter Your Details
                $user="prems.infocom@gmail.com:Infocom@2015"; //your username
                //$mobilenumbers="919XXXXXXXXX"; //enter Mobile numbers comma seperated
                $url="http://api.mvaayoo.com/apidlvr/APIDlvReport";
                ...
                curl_setopt ($ch, CURLOPT_POSTFIELDS,"user=$user&tid=$msgid");
                ...
                if (empty($ret)) {
                        ...
                        if($r[1]=="0"){
                                $b=explode("=",$a[3]);
                                $s=$b[1];    //echo "Message Sent Succesfully" ;
                                if($s=='D') $res= 'DELIVRD';
				else	if($s=='P' || $s=='S'){  $res= $msgid; $st='1'; return true;}
				else	$res= 'Failed';
                           	$st='0'; 
				return true;
                        }else {
	...
	function sms_status_api_1($msgid,$mobiles,&$res,&$st){
		//Please Enter Your Details
		$st='0';
	...
	$obj = new JConfig();
 	$con = mysql_connect("localhost",$obj->user,$obj->password) or die(mysql_error());
		...
		//send sms
		$rs=sms_status_api($row['errortext'],$row['mobile'],$resdata,$st);
		if($rs==true){
				if($st=='1') 
					;
				else
