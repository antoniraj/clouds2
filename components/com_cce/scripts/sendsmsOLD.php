...
<?php
	...
	$obj = new JConfig();
 	$con = mysql_connect("localhost",$obj->user,$obj->password) or die(mysql_error());
		...
		if($rs==true){
                	$q = "INSERT INTO $obj->dbprefix"."sms_sent_status_q(`id`,`smstext`,`mobile`,`stype`,`cdate`,`ctime`,`sid`,`logid`,`aid`,`errorcode`,`errortext`) VALUES('".$row['id']."','".mysql_real_escape_string($row['smstext'])."','".$row['mobile']."','".$row['stype']."','".$row['cdate']."','".$row['ctime']."','".$row['sid']."','".$row['logid']."',(SELECT id FROM ya_academicyears WHERE status='Y'),'0','".mysql_real_escape_string($resdata)."')";
		}else{
