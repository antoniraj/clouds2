...
function sms_balance(&$res){
                ...
                $user="prems.infocom@gmail.com"; //your username
                $password="Infocom@2015"; //your password
                //$mobilenumbers="919XXXXXXXXX"; //enter Mobile numbers comma seperated
                $url="http://api.mvaayoo.com/mvaayooapi/APIUtil";
                //domain name: Domain name Replace With Your Domain  
                $ch = curl_init();
                if (!$ch){
                        //      die("Couldn't initialize a cURL handle");
                        echo "CURL: could not initialize...";
                        return false;
                }
                $ret = curl_setopt($ch, CURLOPT_URL,$url);
                $ret = curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                curl_setopt ($ch, CURLOPT_POST, 1);
                curl_setopt ($ch, CURLOPT_POSTFIELDS,"user=$user:$password&type=0");
                ...
                if (empty($curlresponse)) {
                        ...
                        $x=explode(",",$curlresponse);
                        $rs = explode("=",$x[0]);
                        if($rs[1]=="0"){
                                $a=explode("is",$curlresponse);
                                $res=trim($a[1]);    //echo "Message Sent Succesfully" ;
                                return true;
                        }else return false;
                }
        }

	function sms_balance_1(&$res){
                //Please Enter Your Details
//                $user="williamsundaram"; //your username
  //              $password="54321"; //your password
   ...
   function sms_api($smstext,$mobiles,&$res){
                ...
                curl_setopt ($ch, CURLOPT_POST, 1);
                curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
                curl_setopt ($ch, CURLOPT_POSTFIELDS,"user=$user:$password&senderID=$senderid&receipientno=$mobilenumbers&msgtxt=$message&state=4");
                // Timeout in seconds
                //If you are behind proxy then please uncomment below line and provide your proxy ip with port.
                // $ret = curl_setopt($ch, CURLOPT_PROXY, "PROXY IP ADDRESS:PORT");
                $curlresponse = curl_exec($ch); // execute
	...
	function sms_api_1($smstext,$mobiles,&$res){
		//Please Enter Your Details
                //$user="williamsundaram"; //your username
               // $password="54321"; //your password
 		$user="stannesmatric"; //your username
