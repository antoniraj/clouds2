...
$view = $this->getView($viewName, $viewType, '', array('base_path'=> $this->basePath, 'layout' => $viewLayout));
	...
	function displayhomeworks()
       ...
       function displayexamtimetable()
			...
			if(count($hrecs)>0){
				$sids = array();
					...
					if(strlen($hrec->testportion)>2)
	                        		$smstext=$smstext.'['.$hrec->subjectcode."]".$hrec->testportion.". ";
	                	}
        ...
        function sendexamtimetable(){
                ...
                $redirectTo = JRoute::_('index.php?option='.JRequest::getVar('option').'&controller=sms&view=sms&task=display&layout=smsqueue&flag='.$flag.'&Itemid='.$Itemid.'&logid='.$logid,false);
                $this->setRedirect($redirectTo);
        }
        ...
        function savetestmobile(){
                ...
                $redirectTo = JRoute::_('index.php?option='.JRequest::getVar('option').'&view=sms&controller=sms&task=smsqueue&layout=smsqueue&flag='.$data['flag'].'&testsmsno='.$data['testsmsno'].'&Itemid='.$Itemid,false);
	...
	function deleteexamtimetableentry(){
                $tid= JRequest::getVar('tid');
                $examid= JRequest::getVar('examid');
                $flag= JRequest::getVar('flag');
                $ttid= JRequest::getVar('ttid');
                $model = & $this->getModel('sms');
		$model->deleteSMSTimeTableEntry($tid);
                $redirectTo = JRoute::_('index.php?option='.JRequest::getVar('option').'&view=sms&controller=sms&task=displayexamtimetable&layout=examtimetable&flag='.$flag.'&examid='.$examid.'&ttid='.$ttid.'&Itemid='.$Itemid,false);
                $this->setRedirect($redirectTo);
	}
		...
		$courses=$_POST['courses'];
                ...
                $Itemid = JRequest::getVar('Itemid');
		if($Gocmd=='Go'){
                	$redirectTo = JRoute::_('index.php?option='.JRequest::getVar('option').'&view=sms&controller=sms&task=displayexamtimetable&layout=examtimetable&flag='.$flag.'&examid='.$examid.'&Itemid='.$Itemid,false);
                        $this->setRedirect($redirectTo);
			return;
		...
		if($Newcmd=='New'){
                	$redirectTo = JRoute::_('index.php?option='.JRequest::getVar('option').'&view=sms&controller=sms&task=displayexamtimetable&layout=examtimetable&flag='.$flag.'&examid='.$examid.'&Itemid='.$Itemid,false);
			$ttid = $model->addSMSTimeTableTitle('New Timetable title...',$examid);
			if($ttid){
                		$redirectTo = JRoute::_('index.php?option='.JRequest::getVar('option').'&view=sms&controller=sms&task=displayexamtimetable&layout=examtimetable&flag='.$flag.'&examid='.$examid.'&ttid='.$ttid.'&Itemid='.$Itemid,false);
                        	$this->setRedirect($redirectTo);
			}else{
		...
		if($Updatecmd=="Update"){
                        $redirectTo = JRoute::_('index.php?option='.JRequest::getVar('option').'&view=sms&controller=sms&task=displayexamtimetable&layout=examtimetable&flag='.$flag.'&examid='.$examid.'&ttid='.$ttid.'&Itemid='.$Itemid,false);
                        $rttid = $model->updateSMSTimeTableTitle($tttitle,$ttid);
                        if($rttid){
                                $redirectTo = JRoute::_('index.php?option='.JRequest::getVar('option').'&view=sms&controller=sms&task=displayexamtimetable&layout=examtimetable&flag='.$flag.'&examid='.$examid.'&ttid='.$ttid.'&Itemid='.$Itemid,false);
                                $this->setRedirect($redirectTo,'Updated..');
                        }else{
                                $this->setRedirect($redirectTo,'Failed to update..');
                        }
		}

                if($sendcmd=='Send' || $savecmd=='Save'){
			$smstext='';

			if($ttid){
				$model->deleteSMSTimeTableCourses($ttid);
				foreach($courses as $cid){
					$model->saveSMSTimeTableCourses($ttid,$cid);
				}
			}
			if($ttid&&$examid){
        	        	$fdates= $_POST['fdate'];
                		...
                		$tids= $_POST['tid'];
	                	$redirectTo = JRoute::_('index.php?option='.JRequest::getVar('option').'&view=sms&controller=sms&task=displayexamtimetable&layout=examtimetable&flag='.$flag.'&examid='.$examid.'&ttid='.$ttid.'&Itemid='.$Itemid,false);
				$smstext='Dear Parents,<br /> '.$etitle.'<br />Time Table for '.$tttitle.' ';
				for($i=1;$i<=15;$i++){
					$fdate = $fdates[$i.'$$'.$ttid];
					$fn = $fns[$i.'$$'.$ttid];
					$an = $ans[$i.'$$'.$ttid];
					...
					$a = explode("-",$fdate);
					$fdate=$a[2].'-'.$a[1].'-'.$a[0];
        		                if((($tid=="-1")&&(strlen(trim($fdate))>0))&&((strlen(trim($fn))>0)||(strlen(trim($an))>0))){
                        		        $rs=$model->saveSMSExamTimeTable($i,$fdate,$fn,$an,$ttid);
						$forenoon='';
						$afternoon='';
                                                $ann='0';
                                                $fnn='0';
                                                if(strlen(trim($fn))>1) {
                                                        $forenoon=':[FN:'.htmlspecialchars($fn).']';
                                                        $fnn='1';
                                                }
                                                if(strlen(trim($an))>1){
                                                        $afternoon='[AN:'.htmlspecialchars($an).'] ';
                                                        $ann='1';
                                                }
                                                if($fnn=='1' && $ann=='1' ) $afnn=$forenoon.'|'.$afternoon;
                                                if($fnn=='1' && $ann=='0' ) $afnn=$forenoon.' ';
                                                if($fnn=='0' && $ann=='1' ) $afnn=':'.$afternoon;
                                                if($fnn=='0' && $ann=='0' ) $afnn='';
                                                $smstext=$smstext.'<br />'.JArrayHelper::indianDate($fdate).$afnn;
						//$smstext=$smstext.'<br />'.JArrayHelper::indianDate($fdate).$forenoon.$afternoon;
						//$smstext=$smstext.'<br />'.JArrayHelper::indianDate($fdate).':[FN:'.htmlspecialchars($fn).']|[AN:'.htmlspecialchars($an).'] ';
					}else if($tid!="-1"){
						$forenoon='';
						$afternoon='';
                                                $ann='0';
		                                ...
		                                $rs = $model->updateSMSExamTimeTable($tid,$fdate,$fn,$an);
						//$smstext=$smstext.'<br />'.JArrayHelper::indianDate($fdate).$forenoon.$afternoon;
						//$smstext=$smstext.'<br />'.JArrayHelper::indianDate($fdate).':[FN:'.htmlspecialchars($fn).']|[AN:'.htmlspecialchars($an).'] ';
					}
				}
				$smstext=$smstext.'<br />'.'Principal ';
			}
			if($sendcmd=='Send'){
				$cs = implode(",",$courses);
	                        $redirectTo = JRoute::_('index.php?option='.JRequest::getVar('option').'&controller=sms&view=sms&task=logstudentsbatchsms&layout=examtimetable&flag='.$flag.'&smstext='.$smstext.'&courses='.$cs.'&ttflag=1&Itemid='.$Itemid,false);
  	                        $this->setRedirect($redirectTo);
				return;
			}
		}
...
//Logs for approval from today's absentees report
	...
	if(count($trecs)==0){
                $redirectTo = JRoute::_('index.php?option='.JRequest::getVar('option').'&controller=sms&view=sms&task=smsqueue&layout=smsqueue&Itemid='.$Itemid,false);
                JError::raiseWarning(500,'Sorry, No Absentees');
                $this->setRedirect($redirectTo,'');
                return;
	}
        $sids=array();
     ...
     function sendstudentsms(){
        	$smssentfile = JPATH_COMPONENT.DS.'smstemp'.DS.'smssent.txt';
		touch($smssentfile);
...
//log for approval
    ...
    function logstudentsbatchsms() {
        ...
        $user =& JFactory::getUser();
        $smsdata = JRequest::get('POST');
	$ttflag=JRequest::getVar('ttflag');
	if($ttflag=='1'){
		$smstext=JRequest::getVar('smstext');
		$a = JRequest::getVar('courses');
		$ids = explode(",",$a);
		...
		$logid = $model->logStudentSMS($smsdata['smstext'],$typemsg,$user->username,$corsecodes,$studentsids);
        	$q=$model->updateStudentASMSLog($logid,'A');
