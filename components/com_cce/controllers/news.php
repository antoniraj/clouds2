...
class CCEControllerNews extends JController
{
  ...
  function updatethoughtsfortheday()
    {
                $news = JRequest::get('POST');
                $redirectTo = JRoute::_('index.php?option='.JRequest::getVar('option').'&view=thoughtsfortheday&controller=news&layout=sidenews&Itemid='.$news['Itemid'],false);
                $model = & $this->getModel('news');
                if($news['id']){
                        $status = $model->updatethoughts_day($news['id'],$news['t_days'],$news['message'],$news['reference']);
                }else{
                        $status = $model->savethoughts_day($news['t_days'],$news['message'],$news['reference']);
                }
                if($status==false){
                        JError::raiseWarning(500,'Could not save record..');
                        $this->setRedirect($redirectTo,'');
                }else{
                        $this->setRedirect($redirectTo,'Thought has been updated...');
                }
    }
